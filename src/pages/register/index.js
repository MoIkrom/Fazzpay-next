...
function Register() {
  ...
  const valueFirstname = (e) => {
    setInputpending(false), setInput(true), setFirstname(e.target.value);
  };

  const valueLastname = (e) => {
    setInputpending_(false), setInput_(true), setLastname(e.target.value);
  };

  const valuePassword = (e) => {
    setInputpending__(false), setInput__(true), setPassword(e.target.value);
  };

  const valueEmail = (e) => {
    setInputemail(true), setInputpendingemail(false), setEmail(e.target.value);
  };
  ...
  const postRegister = () => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/register`, { firstName: firstname, lastName: lastname, email: email, password: password })
      .then((res) => {
        toast.success(`register success ${firstname}`), toast.success(`Please Check your email to verification`), setInputpending(true), setInputpendingemail(true), setTimeout(() => router.push("/login"), 3000);
      })

      .catch((err) => {
        toast.error(errorMessage), setInputemail(false), setInputpendingemail(false), setInput(true), setInputpending(true), setInput_(true), setInputpending_(true), setInput__(true), setInputpending__(true);
      });
  };
        ...
        <div className={css.content_right}>
          <div className={css.content_form}>
            ...
            <div className={css.firstname}>
              ...
              <input type="text" name="firstName" id="" onChange={valueFirstname} placeholder="Enter your firstname" />
            </div>
            <div className={css.lastname}>
              <i className={`fa-solid fa-user ${inputpending_ ? "text-secondary" : input_ ? "text-primary" : "text-danger"}`}></i>
              <input type="text" name="lastName" id="" onChange={valueLastname} placeholder="Enter your lastname" />
            </div>
            <div className={css.email}>
              <i className={`fa-regular fa-envelope ${inputpendingemail ? "text-secondary" : inputemail ? "text-primary" : "text-danger"}`}></i>
              <input type="email" name="email" id="" onChange={valueEmail} placeholder="Enter your e-mail" />
            </div>
            <div className={css.password}>
              <i className={`fa-solid fa-lock ${inputpending__ ? "text-secondary" : input__ ? "text-primary" : "text-danger"}`}></i>
              <input type={type} name="password" id="" onChange={valuePassword} placeholder="Enter your password" />
